...
import os, time, mouse, random, keyboard
import webbrowser
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cm = ""
    for i in COMMAND_CATEGORIES: cm = cm + i
...
async def move_direction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"{pos[1] - step if 'up' in command else pos[1] + step if 'down' in command else pos[1]})"
    )
# КЛИКИ мышью - отдельная группан"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Система
        ...
        result = ("Выключаю компьютер и свет досвидания!")
    ...
    # Системная информация
        ...
        if battery:
            if battery.power_plugged:
                ...
                # Батарея не заряжается — показываем, сколько осталось
                if battery.secsleft == psutil.POWER_TIME_UNLIMITED:
                    result = f"Батарея не заряжаеться. Уровень: {battery.percent}%"
                    ...
                    result = f"Батарея не заряжаеться. Уровень: {battery.percent}%. Осталось примерно {hours} часов {minutes} минут."
        else:
